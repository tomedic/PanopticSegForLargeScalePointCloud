...
class Base3DMatch(Dataset):
    ...
    dict_urls = dict(
        train=list_urls_train, train_small=list_urls_train_small, train_tiny=list_urls_train_tiny, val=list_urls_val
    )
    ...
    def __init__(
        ...
        if mode not in self.dict_urls.keys():
            raise RuntimeError("this mode {} does " "not exist" "(train_small|train_tiny|train|val|test)".format(mode))
    ...
    def processed_file_names(self):
        res = [osp.join(self.mode, "raw_fragment"), osp.join(self.mode, "matches"), osp.join(self.mode, "fragment")]
        if self.is_offline:
    ...
    def _create_fragment(self, mod):
        ...
        for scene_path in os.listdir(osp.join(self.raw_dir, mod)):
            ...
            list_seq = [f for f in os.listdir(osp.join(self.raw_dir, mod, scene_path)) if "seq" in f]
            for seq in list_seq:
                frames_dir = osp.join(self.raw_dir, self.mode, scene_path, seq)
                out_dir = osp.join(self.processed_dir, mod, "raw_fragment", scene_path, seq)
                makedirs(out_dir)
                path_intrinsic = osp.join(self.raw_dir, self.mode, scene_path, "camera-intrinsics.txt")
                list_path_frames = sorted(
                    [osp.join(frames_dir, f) for f in os.listdir(frames_dir) if "png" in f and "depth" in f]
                )
                # list_path_color = sorted([osp.join(frames_dir, f)
                #                          for f in os.listdir(frames_dir)
                #                          if 'png' in f and 'color' in f])
                list_path_trans = sorted(
                    [osp.join(frames_dir, f) for f in os.listdir(frames_dir) if "pose" in f and "txt" in f]
                )
    ...
    def _pre_transform_fragment(self, mod):
        ...
        for scene_path in os.listdir(osp.join(self.raw_dir, mod)):
            ...
            list_seq = [f for f in os.listdir(osp.join(self.raw_dir, mod, scene_path)) if "seq" in f]
            for seq in list_seq:
                in_dir = osp.join(self.processed_dir, mod, "raw_fragment", scene_path, seq)
                out_dir = osp.join(self.processed_dir, mod, "fragment", scene_path, seq)
                makedirs(out_dir)
                list_fragment_path = sorted([f for f in os.listdir(in_dir) if "fragment" in f])
    ...
    def _compute_matches_between_fragments(self, mod):
        ...
        for scene_path in os.listdir(osp.join(self.raw_dir, mod)):
            ...
            list_seq = sorted([f for f in os.listdir(osp.join(self.raw_dir, mod, scene_path)) if "seq" in f])
            for seq in list_seq:
                log.info("{}, {}".format(scene_path, seq))
                fragment_dir = osp.join(self.processed_dir, mod, "fragment", scene_path, seq)
                list_fragment_path = sorted(
                    [osp.join(fragment_dir, f) for f in os.listdir(fragment_dir) if "fragment" in f]
                )
    ...
    def _save_patches(self, mod):
        ...
        for i in range(len(os.listdir(match_dir))):
            match = np.load(osp.join(match_dir, "matches{:06d}.npy".format(i)), allow_pickle=True).item()
            ...
            for _ in range(self.num_random_pt):
                ...
                if self.pre_filter is not None:
                    if self.pre_filter(data_source) and self.pre_filter(data_target):
                        ...
                        torch.save(data_source, osp.join(out_dir, "patches_source{:06d}.pt".format(idx)))
                        torch.save(data_target, osp.join(out_dir, "patches_target{:06d}.pt".format(idx)))
                        idx += 1
                else:
                    torch.save(data_source, osp.join(out_dir, "patches_source{:06d}.pt".format(idx)))
                    torch.save(data_target, osp.join(out_dir, "patches_target{:06d}.pt".format(idx)))
