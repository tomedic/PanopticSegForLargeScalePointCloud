...
class Test3DMatch(Base3DMatchTest):
    ...
    def __init__(
        ...
        super(Test3DMatch, self).__init__(root, transform, pre_transform, pre_filter, verbose, debug, max_dist_overlap)
    ...
    def get_patches(self, idx):
        fragment = torch.load(osp.join(self.path_table, "fragment_{:06d}.pt".format(idx)))
        patch_dataset = [self.patch_extractor(fragment, fragment.keypoints[i]) for i in range(self.num_random_pt)]
