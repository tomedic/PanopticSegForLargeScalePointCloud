...
from torch_points3d.datasets.segmentation.wheat3d import Wheat3dSphere, Wheat3dCylinder, INV_OBJECT_LABEL
...
def final_eval(pre_sem, pre_ins_embed, pre_ins_offset, pos, gt_sem, gt_ins):
    ...
    # Keep points that belong to instance ("thing") classes, generically derived
    # from the module-level SemIDforInstance list. Note: semantics were shifted by +1 above,
    # so shift the instance class ids accordingly.
    inst_shifted = np.asarray(SemIDforInstance, dtype=np.int64) + 1
    idxc = np.isin(gt_sem_complete, inst_shifted) | np.isin(pred_sem_complete, inst_shifted)
...
class PanopticWheat3dSphere(PanopticWheat3dBase, Wheat3dSphere):
    def process(self):
        super().process()
...
class PanopticWheat3dCylinder(PanopticWheat3dBase, Wheat3dCylinder):
    def process(self):
        super().process()
...
class Wheat3dDataset(BaseDataset):
    ...
    def __init__(self, dataset_opt):
        ...
        dataset_cls = PanopticWheat3dCylinder if sampling_format == "cylinder" else PanopticWheat3dSphere

        # Treat fold=None like the default training case (no area hold-out)
        if isinstance(self.dataset_opt.fold, int) or self.dataset_opt.fold is None:
